...
const Inventory = () => {
  ...
  const callGetAllItems = async () => {
    try {
      ...
      axios
        .get("https://www.shreyacollection.in/api/products")
        .then((res) => {
          const products = res.data.products;
          const currentPage = res.data.currentPage;

          // Do something with the data
          console.log(products, currentPage, "res");
        })
        .catch((error) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (salePrice) => (
        <div>
          <div className="text-base font-medium">₹{salePrice?.salePrice}</div>
    ...
    },
    {
      ...
      width: "12%",
      sorter: (a, b) =>
        a?.purchasePrice?.purchasePrice - b?.purchasePrice?.purchasePrice,
      render: (price) => `₹${price}`,
    },
    {
      title: "Stock",
      ...
      render: (stock) => (
        <Tooltip title={`Location: ${stock?.location}`}>
          <div>
            ...
            <div className="text-sm">Min: {stock?.minimumStock}</div>
          </div>
        </Tooltip>
      ),
    },
